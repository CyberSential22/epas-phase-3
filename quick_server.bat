...
echo   Phase 2: Event Submission ^& Database Integration
echo  ══════════════════════════════════════════════════════════
echo.
...
python -c "from app import create_app, db; app = create_app('default'); ctx = app.app_context(); ctx.push(); db.create_all(); print('       Tables OK.'); ctx.pop()" 2>nul
if %errorlevel% neq 0 (
...
echo   Form: http://127.0.0.1:5000/events/create
echo.
